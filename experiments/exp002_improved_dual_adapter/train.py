...
def main():
    ...
    logger.info("="*80)
    logger.info("Strategy: Two-Phase Training")
    logger.info(f"  Phase 1: Global adapter only ({config['training']['phase1_rounds']} rounds × {config['training']['phase1_epochs']} epochs)")
    logger.info(f"  Phase 2: Local adapters only ({config['training']['phase2_rounds']} rounds × {config['training']['phase2_epochs']} epochs)")
